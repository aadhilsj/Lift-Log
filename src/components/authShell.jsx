        ...
        ? React.createElement(React.Fragment,null,
            showLeaveConfirm
              ? React.createElement('div',{style:{padding:"12px",borderRadius:10,background:"rgba(60,10,10,.6)",border:"1px solid rgba(180,60,60,.22)"}},
                  React.createElement('div',{style:{fontSize:12,color:"rgba(200,160,160,.8)",marginBottom:10,lineHeight:1.5}},`Leave this Bloc? You won't pay or collect anything this month.`),
...
const authTitle = (step, mode, intent) => {
  if (step === "confirmNewAccount") return "Check the address";
...
const authHelper = (step, mode, intent, email) => {
  ...
  // Names the address rather than asking "are you sure": a mistyped domain is
  // only caught by being read, and by this point the typist has stopped
  // looking at the field.
  if (step === "confirmNewAccount") return `We'll send a code to ${email} and set up a new account.`;
...
const AuthFlowModal = ({step,mode="signin",intent="",email,setEmail,code,setCode,displayName,setDisplayName,onClose,onSendOtp,onVerifyOtp,onSaveProfile,onConfirmExistingAccount,onUseDifferentEmail,onRetryAccountSync,retryingAccountSync,resendCooldown=0,offerAccountCreation=false,onOfferCreateAccount,onConfirmNewAccount,onFixEmailBeforeSignup,sending,verifying,savingProfile,error,devCode}) => renderTopLevelOverlay(React.createElement('div',{className:"overlay center-mobile",onClick:containOverlayActivation,onMouseDown:containOverlayActivation,onPointerDown:containOverlayActivation,onTouchStart:containOverlayTouch,onTouchMove:containOverlayTouchMove,onTouchEnd:containOverlayTouch,onTouchCancel:containOverlayTouch,style:{touchAction:"none",zIndex:10000,pointerEvents:"auto"}},
  React.createElement('div',{className:"modal pi",onClick:e=>e.stopPropagation(),onTouchStart:stopModalTouch,onTouchMove:stopModalTouch,onTouchEnd:stopModalTouch,onTouchCancel:stopModalTouch,style:{maxWidth:420,touchAction:"auto"}},
    ...
    error && resendCooldown <= 0 && React.createElement('div',{style:{fontSize:12,color:"var(--red)",marginBottom:offerAccountCreation&&step==="email"?10:16,whiteSpace:"pre-wrap"}},error),
    step==="email" && offerAccountCreation && React.createElement('button',{onClick:onOfferCreateAccount,style:{width:"100%",background:"transparent",border:"1px solid var(--green)",color:"var(--green)",padding:"12px",borderRadius:10,fontFamily:"'Outfit', sans-serif",fontSize:14,fontWeight:800,marginBottom:16,cursor:"pointer"}},"Create an account with this email"),
    React.createElement('div',{style:{display:"flex",gap:9}},
      step!=="existing" && step!=="alreadyMember" && step!=="confirmNewAccount" && React.createElement('button',{onClick:onClose,style:{flex:1,background:"var(--s2)",border:"1px solid var(--border)",color:"var(--muted)",padding:"14px",borderRadius:10,fontFamily:"'Outfit', sans-serif",fontSize:15,fontWeight:700}},"Cancel"),
      step==="email" && (() => {
      ...
      step==="confirmNewAccount" && React.createElement('button',{onClick:onFixEmailBeforeSignup,style:{flex:1,background:"var(--s2)",border:"1px solid var(--border)",color:"var(--muted)",padding:"14px",borderRadius:10,fontFamily:"'Outfit', sans-serif",fontSize:15,fontWeight:700}},"Fix my email"),
      step==="confirmNewAccount" && React.createElement('button',{disabled:sending,onClick:onConfirmNewAccount,style:{flex:1,background:!sending?"var(--green)":"var(--s3)",color:!sending?"#000":"var(--muted2)",padding:"14px",borderRadius:10,fontSize:15,fontWeight:800}},sending?"Sending...":"Yes, create it"),

            ...
            React.createElement('div', { style: { display: 'grid', gridTemplateColumns: '24px minmax(0,1fr)', gap: 8, alignItems: 'stretch' } },
              ...
              React.createElement('div', { style: { minWidth: 0 } },
                React.createElement('svg', { width: '100%', height: 128, viewBox: '0 0 100 76', preserveAspectRatio: 'none', style: { display: 'block', overflow: 'visible' } },
                  ...
                  React.createElement('polyline', { points, fill: 'none', stroke: '#4ECDC4', strokeWidth: 1.6, strokeLinecap: 'round', strokeLinejoin: 'round', vectorEffect: 'non-scaling-stroke' }),
                  trendMonths.map((month, index) => React.createElement('circle', { key: month.key, cx: trendMonths.length === 1 ? 50 : (index / (trendMonths.length - 1)) * 100, cy: 68 - (month.count / axisMax) * 56, r: selectedKey === month.key ? 2.2 : 1.25, fill: '#fff', stroke: '#071010', strokeWidth: .55, style: { cursor: 'pointer' }, onClick: () => setSelectedKey(key => key === month.key ? null : month.key) }))
                ),

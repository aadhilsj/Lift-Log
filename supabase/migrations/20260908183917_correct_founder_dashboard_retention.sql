-- Correct founder-dashboard retention to use the prior period as its cohort:
-- people active in the prior period who returned in the current period.
-- No product data is changed by this migration.
...
declare
  ...
  v_previous_active_week bigint := 0;
  v_returning_week bigint := 0;
  v_active_month bigint := 0;
  v_previous_active_month bigint := 0;
...
begin
  ...
  select count(distinct profile_id) into v_previous_active_week
  from ante_core.app_daily_activity
  where activity_date between v_previous_week_start and (v_week_start - 1);

  select count(distinct previous_period.profile_id) into v_returning_week
  from ante_core.app_daily_activity previous_period
  where previous_period.activity_date between v_previous_week_start and (v_week_start - 1)
    and exists (
      select 1 from ante_core.app_daily_activity current_period
      where current_period.profile_id = previous_period.profile_id
        and current_period.activity_date between v_week_start and v_today
    );
  ...
  select count(distinct profile_id) into v_previous_active_month
  from ante_core.app_daily_activity
  where activity_date >= v_previous_month_start
    and activity_date < v_month_start;

  select count(distinct previous_period.profile_id) into v_returning_month
  from ante_core.app_daily_activity previous_period
  where previous_period.activity_date >= v_previous_month_start
    and previous_period.activity_date < v_month_start
    and exists (
      select 1 from ante_core.app_daily_activity current_period
      where current_period.profile_id = previous_period.profile_id
        and current_period.activity_date between v_month_start and v_today
    );
  ...
  return jsonb_build_object(
    ...
    'retention', jsonb_build_object(
      'weekly', jsonb_build_object(
        'activeUsers', v_active_week,
        'previousPeriodActiveUsers', v_previous_active_week,
        'returningUsers', v_returning_week,
        'rate', case when v_previous_active_week = 0 then 0 else round((v_returning_week::numeric * 100) / v_previous_active_week, 1) end
      ),
      'monthly', jsonb_build_object(
        'activeUsers', v_active_month,
        'previousPeriodActiveUsers', v_previous_active_month,
        'returningUsers', v_returning_month,
        'rate', case when v_previous_active_month = 0 then 0 else round((v_returning_month::numeric * 100) / v_previous_active_month, 1) end
      )
    ),
